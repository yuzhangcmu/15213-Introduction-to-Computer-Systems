...
// printf("x:%.8x\n");
  ...
  // exp initialized to be 158(127+31), for cases when x=0x0 or INT_MIN
  int exp = (x >> 31) ? 158 : 0;
  int frac = 0;
  int delta;
  int frac_mask;
...
// printf("---Found E:%d\n", E);
  ...
  // Set exp bits to the right place
  res |= (exp << 23);
  // Set frac bits to the right place
  res |= frac;
// printf("res:%.8x\n", res);
  return res;
}
